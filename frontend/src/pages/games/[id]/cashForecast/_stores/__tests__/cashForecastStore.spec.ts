...
describe("cashForecastEventStore", () => {
  ...
  describe("eventApplier", () => {
    ...
    it("AddLoan", () => {
      ...
      expect(applied.loans.list[0].repaymentTerms).toBeNull();
    ...
    describe("LoanEvents", () => {
      ...
      it("SetLoanCashInDate on first CashIn", () => {
        ...
        const applied = eventApplier.apply(model, event);
        expect(applied.loans.list[0].cashIn.list[0].date.value).toBe(expectedDate);
